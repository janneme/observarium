<script>
  ...
  const ITEM_H = 44
  const PAD_H  = 44   // (132 px drum height − 44 px item) / 2
  ...
  function isSel(d) { return d && selDate.getFullYear() === calYear && selDate.getMonth() === calMonth && selDate.getDate() === d }
  function isTdy(d) { return d && today.getFullYear()   === calYear && today.getMonth()   === calMonth && today.getDate()   === d }

  function prevMonth() { if (calMonth === 0) { calYear--; calMonth = 11 } else calMonth-- }
  function nextMonth() { if (calMonth === 11) { calYear++; calMonth = 0  } else calMonth++ }
  function selectDay(d) { if (d) selDate = new Date(calYear, calMonth, d) }
  ...
  function onHourScroll()   { if (hourEl)   hourIdx   = Math.max(0, Math.min(23, Math.round(hourEl.scrollTop   / ITEM_H))) }
  function onMinuteScroll() { if (minuteEl) minuteIdx = Math.max(0, Math.min(11, Math.round(minuteEl.scrollTop / ITEM_H))) }
  ...
  function drumWheel(e, acc, el, count) {
    e.stopPropagation()
    e.preventDefault()
    const px = e.deltaMode === 1 ? e.deltaY * ITEM_H
             : e.deltaMode === 2 ? e.deltaY * 132
             : e.deltaY
    acc.v += px
  ...
  function apply()  { dispatch('pick', buildResult()) }
  function cancel() { dispatch('cancel') }

  async function useNow() {
    const now = new Date()
    selDate  = new Date(now); selDate.setHours(0,0,0,0)
...
<div class="panel" on:pointerdown|stopPropagation>
  ...
  <div class="picker-body">
    ...
    <div class="time-section">
      ...
      <div class="drum-row">
        <div class="drum" bind:this={hourEl}
        ...
        <div class="drum" bind:this={minuteEl}
...
.sheet-title { font-size: 0.88rem; opacity: 0.75; }
...
.action-btn:hover { background: none; transform: none; }
.action-btn.primary { opacity: 1; font-weight: 600; }
...
.nav-btn:hover { background: none; opacity: 1; transform: none; }
...
.drum-row::before { top: 44px; }
.drum-row::after  { top: 88px; }
...
.drum-item.sel { opacity: 1; font-weight: 600; }
...
.now-btn:hover { background: rgba(255,255,255,0.08); transform: none; }
</style>

...
<div class="top-bar" on:pointerdown|stopPropagation>
  ...
  {#if $selectedObject}
    <div class="row2" role="button" tabindex="0" on:click={() => selectedObject.set(null)} on:keydown={(e) => e.key === 'Enter' && selectedObject.set(null)}>
...
.dt-date { opacity: 0.6; }
.dt-sep { width: 0.4em; }
.dt-time { font-variant-numeric: tabular-nums; }
...
.row2:hover { background: rgba(127,127,127,0.06); }

.obj-icon { font-size: 0.85rem; opacity: 0.75; flex-shrink: 0; }
.obj-name { flex: 1; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.obj-const { opacity: 0.45; font-size: 0.75rem; flex-shrink: 0; }
</style>

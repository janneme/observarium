...
## 1. Stars — AT-HYG v3.3 (Augmented Tycho-HYG)

| Field              | Value                                                         |
| ------------------ | ------------------------------------------------------------- |
| **Canonical repo** | <https://codeberg.org/astronexus/athyg>                       |
| **File used**      | `data/subsets/athyg_33_reduced_m11.csv.gz` (subset, mag ≤ 11) |
| **Licence**        | CC-BY-SA 4.0                                                  |
| **Record count**   | 871,139 stars (complete to V = 11, mostly to V = 11.5)        |

**Description:** Merges Tycho-2 with Gaia DR3 distances/velocities and the
original HYG catalogue (Hipparcos + Yale Bright Star + Gliese). Provides
accurate positions and distances for over 2 M stars. Key columns used: `hip`,
`hd`, `proper`, `ra`, `dec`, `mag`, `spect`, `dist` (parsecs), `con`
(IAU 3-letter constellation abbreviation).

**Quirks:**

- File is stored in Git LFS on Codeberg. Use the `/media/` URL endpoint
  (not `/raw/`) to download actual content rather than the LFS pointer.
- No `sao`, `var_min`, or `var_max` columns — variable star magnitude ranges
  are not currently part of AT-HYG.
- `con` field already contains the IAU constellation assignment — no
  point-in-polygon pass needed to assign stars to constellations.
- Europe visibility filter: keep `dec >= -35°`.

**Previous source:** HYG v4.2 (same author, CC-BY-SA 4.0). Replaced because
HYG is complete only to ~mag 9 (Hipparcos limit); AT-HYG extends coverage to
mag 11 with Gaia-quality distances.
